...
- **`-ip auto`（新默认）**：候选表同时含注册信息的 IPv4 与 IPv6 边缘，顺序
  `v4:443 → v6:443 → v4 其余端口 → v6 其余端口`——443 是 QUIC 最标准端口，
  两族 443 先测能最快区分「地址族被限」还是「端口被限」。背景：CT103 实测
  IPv4 边缘（162.159.198.2）QUIC/UDP 全端口被运营商 QoS 而 IPv6 正常，旧默认
  `-ip 4` 把隧道一起带死；新默认启动即自动跨族实测切换，无需人工判断 `-ip 6`。
- **轮内自动切换**：当前候选失败 → 自动尝试下一候选（含跨地址族 v4↔v6）；
  运行中失联（出口探测失败 / QUIC 隧道关闭）重连时同样遍历全表。某地址族
  无路由（ENETUNREACH 等）时只跳过该族剩余候选、继续尝试另一族（原实现
  break 整轮，跨族候选表下会把可达族一起放弃）。
- **不无限重试（语义不变）**：切换是单轮内的候选遍历，连续 3 轮全失败仍进
  30 分钟长退避 + 静默（`dial_retry.go` 的 dialRetryPolicy 未动）。
- **切换日志**：跨族切换打一条边界日志（`IPv4 边缘不可达 → 尝试 IPv6 边缘`），
  每轮只报第一条；长退避静默期内连边界日志也静默。
- **兼容性**：显式 `-ip 4` / `-ip 6` / `-ip host:port` 行为完全不变；Android
  桥（无旗标、双空路径）随之获得 auto 语义；`-scan` 与 auto 不叠加（auto 已含
  拨号层实测，叠加只会拖慢冷启动），组合时提示忽略；成功候选记忆沿用内存
  addrIdx（重连从上次成功候选开始），不写 reg.json。
- 新增测试：auto 候选生成（交错序/443 提前/单族退化/端点全缺报错/显式族不变）
  与轮内切换（跨族续拨/边界日志恰 1 条/quiet 全静默）。

### 修复（拨号长退避，recvu2HKHM5zIj，2026-09-02 CT103 事故）

- **连续 3 轮拨号失败即停**（f27bf4b）：两处拨号循环（装配
  `NewMasqueClientContext` + 重连航班 `runReconnect`）原为指数退避
  100ms→5s 封顶后**永不停止**——CT103 上边缘 QUIC 全端口不可达时每 5s
  一轮 × 每轮 7+ 条日志紧循环刷 journal 4 天（叠加 qlog 泄漏引爆 tmpfs，
  触发宿主 IO 风暴两次）。新语义（东哥拍板「重试三次不通及停」）：
  - 一「轮」= 一次完整 `dial`（扫全部候选边缘地址，全失败记 1 轮）；
  - 连续 3 轮全失败 → 停止密集重试，切入 **30 分钟长退避**（低频探测，
    恢复后自动回到快速节奏，无需重启）；
  - 每个故障期只打 1 条「进入长退避」边界日志，退避期静默逐端口过程
    日志（`dial`/`dialAddr` 增加 `quiet` 参数）；
  - 重连航班成功重建后复位计数，下次拆线从快速节奏重新开始。
- 新增 `tunnel/dial_retry.go`（`dialRetryPolicy`：两循环共用的重试节奏）
  与配套单测（策略表驱动 + 两循环端到端行为锁，共 6 个测试）。
