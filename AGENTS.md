...
| M9.16 分应用代理 + v0.6.0 发布（2026-08-27） | ✅ | **分应用代理**（recvtpWe，东哥拍板 v1.0 方案，计划文档 `.omo/plans/per-app-proxy-2026-08-27.md`）：VpnService 按应用包名分流（PER_APP_PROXY 模式），精确控制哪些应用走隧道——替代全局代理一刀切。改动面（f69b370）：`core/config` 新增 per_app 配置项+单测；`gui/service.go`+`androidbridge.go`+`androidctl_other.go` 后端接口（`listInstalledApps` 只列声明 INTERNET 权限的应用 + 配置读写桥）；Android 原生 `WarpVpnService` 按包名分流 + `MainActivity` 选择入口 + Manifest 权限；前端 `SettingsPage` 设置区 + `PerAppPicker` 底部弹层（已选置顶→第三方→系统分组、搜索过滤、壳自身包名剔除防路由死锁）。**入口修复**（9a4b6bb）：Wails v3 Android runtime 不注入 `window._wails.environment`，`System.IsAndroid()` 在 Android 上恒 false → Card 不渲染；去 isAndroid 门控 Card 始终显示（桌面端 config 字段被忽略无副作用）。**发布闭环**：CHANGELOG 定版 [v0.6.0] - 2026-08-27（tag v0.6.0），CI Build and Release 全 job 全绿含 build-android；东哥真机覆盖安装 `warp-perapp-fixed.apk`（9a4b6bb）实测通过；APK 经老马飞书投递协作群。**技术要点**：Android 上做任何"仅移动端 UI 条件渲染"不得依赖 `System.IsAndroid()`（runtime 不注入 env）——用平台能力探测（如 API 存在性）或恒显 + config 字段忽略 |
...
- **v0.5.31 GUI 两修复要点（2026-08-26，recvtkxq）**：① **主题**：`useTheme()` 只能在
  ...
  测试注入点 `geoUpdateFn`
- **v0.6.0 分应用代理（2026-08-27，recvtpWe）**：VpnService 按应用包名分流（PER_APP_PROXY
  模式）。**改 Android 侧前必读**：Android 上**不得用 `System.IsAndroid()` 做条件渲染**——
  Wails v3 Android runtime 不注入 `window._wails.environment`，该 API 恒 false（9a4b6bb
  教训：Card 入口因此不渲染，后改为恒显 + 桌面端忽略 config 字段）。后端 `listInstalledApps`
  只列声明 INTERNET 权限的应用，壳自身包名由上层过滤剔除（防路由死锁）。PerAppPicker
  分组：已选置顶→第三方→系统应用（默认折叠），搜索按名称/包名实时过滤。config 字段
  （per_app_proxy mode + packages）桌面端被忽略无副作用。
